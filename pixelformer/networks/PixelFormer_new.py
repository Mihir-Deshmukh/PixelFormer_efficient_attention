...
from .PQI import PSP
# from .SAM_fast import SAM
from .SAM_Efficient import SAM
########################################################################################################################
...
class PixelFormer(nn.Module):
    ...
    def __init__(self, version=None, inv_depth=False, pretrained=None, 
        ...
        self.sam4 = SAM(input_dim=in_channels[3], embed_dim=sam_dims[3], key_dim = in_channels[3], v_dim=v_dims[3], num_heads=32)
        self.sam3 = SAM(input_dim=in_channels[2], embed_dim=sam_dims[2], key_dim = in_channels[2], v_dim=v_dims[2], num_heads=16)
        self.sam2 = SAM(input_dim=in_channels[1], embed_dim=sam_dims[1], key_dim = in_channels[1], v_dim=v_dims[1], num_heads=8)
        self.sam1 = SAM(input_dim=in_channels[0], embed_dim=sam_dims[0], key_dim = in_channels[0], v_dim=v_dims[0], num_heads=4)
